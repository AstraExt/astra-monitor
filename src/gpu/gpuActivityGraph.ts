...
type GpuActivityGraphProps = GraphProps & {
...
export default GObject.registerClass(
    class GpuActivityGraph extends GraphBase<HistoryData> {
        ...
        constructor(params: GpuActivityGraphProps) {
            super(params);
            ...
            Config.connect(
                this,
                'changed::gpu-header-activity-graph-color1',
                this.setStyle.bind(this)
            );
            //Config.connect(this, 'changed::gpu-header-activity-graph-color2', this.setStyle.bind(this));
        }
        ...
        setStyle() {
            ...
            this.colors = [
                Utils.parseRGBA(
                    Config.get_string('gpu-header-activity-graph-color1'),
                    'rgba(29,172,214,1.0)'
                ),
                //Utils.parseRGBA(Config.get_string('gpu-header-activity-graph-color2'), 'rgba(214,29,29,1.0)')
            ];
        ...
        repaint() {
            ...
            if(this.history && this.history.length > 0) {
                ...
                const totalFunc = (node: HistoryData) => {
                    ...
                    if(!gpuData || !gpuData.activity || Number.isNaN(gpuData.activity.GFX))
                        return 0;
                    return (gpuData.activity.GFX ?? 0) / 100.0;
                };
