...
export default class XMLParser {
    ...
    public parse(xml: string): XMLObject | undefined {
        ...
        while(this.pos < this.xml.length) {
            ...
            if(this.xml[this.pos] === '<') {
                if(this.xml[this.pos + 1] === '/') { // Closing tag
                    ...
                    if(!this.parseTag())
                        break; // Could not find tag: malformed XML
                    ...
                    if(!this.currentObj) {
                        ...
                        if(!this.currentObj[this.currentTagName])
                            ...
                            this.currentObj[this.currentTagName] = [(this.currentObj[this.currentTagName] as XMLObject), newObj];
                    }
    ...
    private skipDeclarations(): void {
        ...
        // Skip whitespace
        while(this.xml[this.pos] === ' ' || this.xml[this.pos] === '\n' || this.xml[this.pos] === '\t' || this.xml[this.pos] === '\r')
            this.pos++;
    ...
    private parseAttributes(): XMLObject {
        ...
        while(this.pos < this.xml.length && this.xml[this.pos] !== '>') {
            ...
            if(this.xml[this.pos] === '=' || this.xml[this.pos] === '"' || this.xml[this.pos] === '\'') {
                if(this.xml[this.pos] === '=') {
                    this.pos++; // Move past '='
                    while(this.xml[this.pos] === ' ')
                        this.pos++; // Skip spaces after '='
                }
                ...
                else { // Handle as number or boolean
            ...
            else { // No value, boolean attribute
                attrs[attrName] = true;
            }
