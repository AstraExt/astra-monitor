...
class StorageGraph extends GraphBase<StorageIO> {
    ...
    constructor(params: StorageGraphProps) {
        ...
        Config.connect(this, 'changed::storage-header-io-graph-color1', this.setStyle.bind(this));
        Config.connect(this, 'changed::storage-header-io-graph-color2', this.setStyle.bind(this));
    }
    ...
    setStyle() {
        ...
        this.colors = [
            Utils.parseRGBA(Config.get_string('storage-header-io-graph-color1'), 'rgba(29,172,214,1.0)'),
            Utils.parseRGBA(Config.get_string('storage-header-io-graph-color2'), 'rgba(214,29,29,1.0)')
        ];
        ...
        if(this.maxReadSpeedLabel) {
            if(lightTheme)
                ...
                this.maxReadSpeedLabel.style_class = 'astra-monitor-graph-label';
        }
        ...
        if(this.maxWriteSpeedLabel) {
            if(lightTheme)
                ...
                this.maxWriteSpeedLabel.style_class = 'astra-monitor-graph-label';
        }
    ...
    buildHistoryGrid() {
        ...
        Config.connect(this, 'changed::storage-io-unit', () => {
            ...
            const maxRead = Math.max(reads.reduce((max, n) => Math.max(max, n), 0), 1000 * 1000);
            
            const writes = Utils.movingAverage(slicedHistory.map(storageIO => storageIO.bytesWrittenPerSec), this.mini ? 2 : 4);
            const maxWrite = Math.max(writes.reduce((max, n) => Math.max(max, n), 0), 1000 * 1000);
