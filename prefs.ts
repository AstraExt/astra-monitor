...
export default class AstraMonitorPrefs extends ExtensionPreferences {
  ...
  fillPreferencesWindow(window: Adw.PreferencesWindow) {
    ...
    if (defaultCategory) {
      ...
      else if (defaultCategory === 'sensors') window.set_visible_page(sensorsPage);
      else if (defaultCategory === 'about') window.set_visible_page(aboutPage);
    }
  ...
  private setupGeneral() {
    ...
    const statusLabel = this.addStatusLabel(
    ...
    this.addComboRow(
      {
        ...
        subtitle:
          _('Set this if you moved your shell to another position to improve the layout.') +
          '\n' +
  ...
  private setupSensors() {
    ...
    const generateSensorSources = async () => {
      ...
      const choicesSource = [{ value: '', text: _('None') }];
      for (const source of sources) choicesSource.push({ value: source.value, text: source.text });
  ...
  private addExpanderRow(props: RowProps, group: Adw.PreferencesGroup | Adw.ExpanderRow) {
    ...
    section.connect('notify::expanded', (widget) => {
      if (widget.expanded) {
        if (this.expanded && this.expanded !== widget) this.expanded.expanded = false;
        this.expanded = widget;
      } else {
        if (this.expanded === widget) this.expanded = null;
      }
    });
    ...
    const addChoices = (choices: Choice[]) => {
      ...
      select.connect('notify::selected', (widget) => {
        ...
        if (selectedChoice !== undefined) {
          ...
          if (type === 'json') Config.set(setting, JSON.stringify(selectedChoice.value), 'string');
          else Config.set(setting, selectedChoice.value, type);
        }
      });
    };
  ...
  OrderingItem(
    ...
    if (buttonDown) row.add_suffix(buttonDown);
    if (buttonUp) row.add_suffix(buttonUp);
